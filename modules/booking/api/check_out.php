...
if ($booking['status'] !== 'checked_in') { echo json_encode(['success' => false, 'message' => 'Must check in first']); exit; }

// Update check-out
$stmt = $conn->prepare("UPDATE Booking SET check_out_time = NOW(), status = 'completed' WHERE booking_id = ?");
$stmt->bind_param("i", $booking_id);
$success = $stmt->execute();
$stmt->close();
$conn->close();

echo json_encode(['success' => $success, 'message' => $success ? 'Checked out successfully' : 'Check-out failed']);
