...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_vehicle'])) {
    ...
    $stmt = $conn->prepare("UPDATE Vehicle SET approved_by = ?, Approved_date = NOW(), status = 'approved' WHERE vehicle_id = ?");
...
<head>
    ...
    <title><?php 
        if ($current_view === 'dashboard') echo 'Staff Dashboard';
        elseif ($current_view === 'profile') echo 'My Profile';
        else echo 'Vehicle Requests';
    ?> - Mawgifi</title>
...
<body>
    <nav class="navbar">
        ...
        <div class="nav-links">
            ...
            <a href="Stafe.php?view=requests" <?php echo $current_view === 'requests' ? 'class="active"' : ''; ?>>Vehicles Request</a>
                ...
                <form method="POST" action="" style="max-width: 600px;">
                    <div class="cards-grid" style="grid-template-columns: 1fr 1fr; margin-bottom: 20px;">
                        <div style="display: flex; flex-direction: column;">
                            ...
                            <input type="text" name="username" value="<?php echo htmlspecialchars($user_data['UserName'] ?? ''); ?>" required style="padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px;">
                        </div>

                        <div style="display: flex; flex-direction: column;">
                            <label style="font-weight: 600; margin-bottom: 8px;">Email Address</label>
                            <input type="email" name="email" value="<?php echo htmlspecialchars($user_data['Email'] ?? ''); ?>" required style="padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px;">
                        </div>
                    </div>

                    <div style="margin-bottom: 20px;">
                        <label style="font-weight: 600; margin-bottom: 8px; display: block;">Phone Number</label>
                        <input type="text" name="phone" value="<?php echo htmlspecialchars($user_data['PhoneNumber'] ?? ''); ?>" placeholder="Enter phone number" style="padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px; width: 100%; max-width: 280px;">
                    </div>
                    ...
                    <div class="cards-grid" style="grid-template-columns: 1fr 1fr; margin-bottom: 20px;">
                        <div style="display: flex; flex-direction: column;">
                            ...
                            <input type="password" name="password" placeholder="Enter new password" style="padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px;">
                        </div>

                        <div style="display: flex; flex-direction: column;">
                            <label style="font-weight: 600; margin-bottom: 8px;">Confirm Password</label>
                            <input type="password" name="confirm_password" placeholder="Confirm new password" style="padding: 12px; border: 2px solid #e2e8f0; border-radius: 10px; font-size: 14px;">
                        </div>
                    </div>

                    <button type="submit" name="update_profile" class="btn btn-approve" style="flex: none; padding: 12px 30px;">Update Profile</button>
                </form>
            </div>
        </div>
                        ...
                        <?php foreach ($pending_vehicles as $vehicle): ?>
                            <div class="vehicle-card pending">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="date">Submitted: <?php echo date('M d, Y H:i', strtotime($vehicle['created_at'])); ?></p>
                                </div>
                        ...
                        <tbody>
                            <?php foreach ($approved_vehicles as $vehicle): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($vehicle['vehicle_type'] . ' - ' . $vehicle['vehicle_model']); ?></td>
